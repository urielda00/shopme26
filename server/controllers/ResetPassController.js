...
export const sendLink = async (req, res) => {
    try {
        ...
        if (!user) {
            ...
            return res.status(200).json({ success: true, message: 'If this email exists, a link has been sent.' });
        }
        ...
        const link = `${process.env.CLIENT_URL}/reset-password/${user._id}/${resetToken}`;
...
export const verifyUrl = async (req, res) => {
    try {
        ...
        ResetPassErrorLogger.error(`Error in verifyUrl: ${error.message}`);
        res.status(500).json({ success: false, message: 'Internal server error' });
    }
};
